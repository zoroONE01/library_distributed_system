...
class StatsRepositoryImpl implements StatsRepository {
  ...
  StatsRepositoryImpl(this._statsService, this._managerService);
  ...
  Future<Result<SystemStatsResponseEntity>> getSystemStats() async {
    try {
      ...
      if (response.isSuccessful && response.body != null) {
        ...
        return Success(
          SystemStatsResponseEntity(
            ...
            siteStats: model.siteStats
                .map(
                  (siteModel) => SiteStatsEntity(
                ...
                .toList(),
            popularBooks: model.popularBooks
                .map(
                  (bookModel) =>
      ...
      return Failure(
        Exception('Failed to get system stats: ${response.error}'),
      );
    } catch (e) {
  ...
  getReadersWithStats({int page = 0, int size = 20, String? search}) async {
    try {
      final params = {
        'page': page.toString(),
        'size': size.toString(),
  ...
  Future<Result<SiteStatsEntity>> getSiteStats() async {
    try {
      ...
      return systemStatsResult.fold((systemStats) {
  ...
  Future<Result<SystemStatsEntity>> getBasicSystemStats() async {
    try {
      ...
      return Failure(
        Exception('Failed to get basic system stats: ${response.error}'),
      );
    } catch (e) {
  ...
  ReaderWithStatsEntity _mapReaderWithStatsModelToEntity(
    ReaderWithStatsModel model,
  ) {
