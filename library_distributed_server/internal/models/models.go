...
// UserInfo - Detailed user information for profile
// @Description Detailed user information including permissions
type UserInfo struct {
	ID          string `json:"id" example:"user123"`                               // User ID
	Username    string `json:"username" example:"ThuThu_Q1" validate:"required"`   // Username
	Role        string `json:"role" example:"THUTHU" enums:"THUTHU,QUANLY"`        // User role
	MaCN        string `json:"maCN" example:"Q1"`                                  // Branch code (for THUTHU)
	Permissions string `json:"permissions" example:"SELECT,INSERT,UPDATE,EXECUTE"` // User permissions
}
